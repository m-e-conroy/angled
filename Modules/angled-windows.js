...
angular.module('angled-windows-directives',['ngSanitize','ngAnimate','angled-dragndrop','angled-resizeit'])
	...
	.run(['$templateCache',function($templateCache){
		$templateCache.put('/tmpls/window','<div class="floating-window" id="{{id}}" angled-draggable="{handle: \'div.panel-heading\',opacity: 0.5}"><div class="panel panel-primary" angled-resizable="{handles: \'se\', alsoResize: \'#windowBodyContent\'}"><div class="panel-heading cursor-move" id="windowHeading"><span class="pull-right"><a type="button" id="windowMinimizeBtn" class="btn btn-primary btn-xs" ng-click="rollUp()"><span class="glyphicon" ng-class="{false: \'glyphicon-chevron-up\',true: \'glyphicon-chevron-down\'}[rolledUp]"></span></a></span><span><big ng-bind-html="title"></big></span></div><div id="windowBody"><div class="panel-body" id="windowBodyContent" ng-transclude></div><div class="panel-footer"><small>&nbsp;</small><span class="pull-right glyphicon glyphicon-resize-full ui-resizable-handle ui-resizable-se"></span></div></div></div></div>');
	}]); // end run
